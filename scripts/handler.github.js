    ...
    success: function(response) {
        ...
        for(i = 0; i < len; i++) {
            ...
            if(response[i].type.slice(0,-5) == "Follow") {
                secondbit = 'ing ' + response[i].payload.target.name;
            } else if(response[i].type.slice(0,-5) == "Push") {
                secondbit = 'ed to <a href="' + response[i].repo.url + '">' + response[i].repo.name.slice(11) + '</a><br>commit message: \'' + response[i].payload.commits[0].message + '\'';
            } else if(response[i].type.slice(0,-5) == "Create" || response[i].type.slice(0,-5) == "Watch") {
                secondbit = 'ed new repository: ' + response[i].repo.name.slice(11);
            }
...
$.ajax({
    ...
    success: function(response) {
        ...
        $(response).find('div.simple').each(function() {
            ...
            var message = $(this).find('.title').html();
            message = message.replace(/href="/g,'href="http://github.com');
            ...
            $('#apilog').append(
                '<div class="event github">' +
                    ...
                    '<div class="message">' +
                        '<a href="https://github.com/deplicator"><img class="icon" src="./images/github_icon.png"></a>' +
                        message + //[BUG] all links are relative
                    '</div>' +
                '</div>');
